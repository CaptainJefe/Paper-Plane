...
class GameSceneNew: SKScene, SKPhysicsContactDelegate {
...
//        addChild(label)
    ...
    func createPlatforms(yPosition: CGFloat) {
        ...
        // let yPosition = frame.minY + (frame.midY / 4.0) // + 8 // It appears that because of the sizing of th transition plateforms and how they are spaced, they overlap with one enough, so it takes about an extra 8 pixels to space them properly
...
//        print(platformGroup[1].speed)
        ...
        print(platformLeft.size.height)
        print(platformCount)
    }
...
//        buttonRight.color = .black
    ...
    func restartGame(node: SKSpriteNode) {
        if let scene = GameSceneNew(fileNamed: "GameSceneNew") {
