...
class TitleScreen: SKScene {
	...
	var statsButton = SKSpriteNode(imageNamed: "stats_button")
	...
	var closeButton = SKSpriteNode(imageNamed: "close_button")
	...
	func createButtons() {
		...
		statsButton.position = CGPoint(x: playButton.position.x, y: highScoresButton.position.y - 70)
		...
		optionsButton.position = CGPoint(x: playButton.position.x, y: statsButton.position.y - 70)
		...
		if SavedSettings.shared.areAdsRemoved == false {
	...
	func createHighScores() {
		...
		// Grab top 10
		var displayScores = SavedData.shared.getLeaderboard()
		
		let rankColors: [UIColor] = [
			SKColor(hex: 0xFFD700), // Gold
			SKColor(hex: 0xEBECEB), // Silver
			SKColor(hex: 0xCE8946)  // Bronze
		]
		
		// Layout Constants
		let startY = frame.maxY * 0.8
		let rowHeight = 60 * scaleFactor
		let rankX = frame.maxX * 0.15     // Rank on the left
		let scoreX = frame.maxX * 0.85     // Score on the right
		
		// MARK: 3. The Loop (Generate 10 Rows)
		for i in 0..<10 {
			
			var rankColor = UIColor.white
			if i < rankColors.count {
				rankColor = rankColors[i]
			}
			
			// A. Determine Data for this row
			let rankText = "\(i + 1)."
			var scoreText = "..."
			var scoreColor = UIColor.white
			
			// If we have a real score for this index, use it
			if i < displayScores.count {
				scoreText = "\(displayScores[i])"
			}
			
			// B. Calculate Y Position
			let yPosition = startY - (CGFloat(i) * rowHeight)
			
			// C. Create Rank Label (Left)
			let rankLabel = createLabelWithShadow(
				text: rankText,
				fontName: "Paper Plane Font",
				fontSize: 40 * scaleFactor,
				fontColor: rankColor,
				shadowColor: .black,
				position: CGPoint(x: rankX, y: yPosition),
				zPosiion: 160,
				alpha: 0,
				name: "rankLabel_\(i)"
			)
			// Align text to the Left so "1." and "10." align nicely
			rankLabel.main.horizontalAlignmentMode = .left
			rankLabel.shadow.horizontalAlignmentMode = .left
			
			addChild(rankLabel.main)
			addChild(rankLabel.shadow)
			highScoresUIContainer.append(rankLabel.main)
			highScoresUIContainer.append(rankLabel.shadow)
			
			// D. Create Score Label (Right)
			let scoreLabel = createLabelWithShadow(
				text: scoreText,
				fontName: "Paper Plane Font",
				fontSize: 40 * scaleFactor,
				fontColor: scoreColor,
				shadowColor: .black,
				position: CGPoint(x: scoreX, y: yPosition),
				zPosiion: 160,
				alpha: 0,
				name: "scoreLabel_\(i)"
			)
			// Align score to the Right so numbers stack neatly (e.g. 100 vs 1000)
			scoreLabel.main.horizontalAlignmentMode = .right
			scoreLabel.shadow.horizontalAlignmentMode = .right
			
			addChild(scoreLabel.main)
			addChild(scoreLabel.shadow)
			highScoresUIContainer.append(scoreLabel.main)
			highScoresUIContainer.append(scoreLabel.shadow)
			
			// E. Separator
			// Add separator below the text, unless it's the very last item
			if i < 9 {
				let separator = SKSpriteNode(imageNamed: "separator")
				separator.position = CGPoint(x: frame.midX, y: yPosition - (rowHeight / 1.5))
				separator.size = CGSize(width: frame.width * 0.9, height: separator.size.height * scaleFactor)
				separator.alpha = 0
	...
	func createOptions() {
		...
		print("is sound muted: \(SavedSettings.shared.isSoundMuted)")
		
		if SavedSettings.shared.isSoundMuted {
		...
		for i in 0...2 {
			...
			separator.size = CGSize(width: frame.width * 0.75, height: separator.size.height)
			separator.alpha = 0
		...
		versionInfo.position = CGPoint(x: frame.midX, y: frame.maxY * 0.05)
	...
	func createStats() {
		...
		let totalGamesPlayed = SavedData.shared.getGamesPlayed()
		let highestScore = SavedData.shared.getHighscore()
		let averageScoreLast20 = SavedData.shared.getAverageLast20()
		let totalPlatformsPassed = SavedData.shared.getTotalScore()
		let favoriteLevel: String
		let gamesPlayedLevel1 = SavedData.shared.getGamesPlayedLevel1()
		let gamesPlayedLevel2 = SavedData.shared.getGamesPlayedLevel2()
		let gamesPlayedLevel3 = SavedData.shared.getGamesPlayedLevel3()
		
		let levelStats = [
			("Castle", gamesPlayedLevel1),
			("Chasm",  gamesPlayedLevel2),
			("Silo",   gamesPlayedLevel3)
		]
		
		let mostPlayedLevel = levelStats.max { $0.1 < $1.1 }
		
		if let mostPlayed = mostPlayedLevel, mostPlayed.1 > 0 {
			favoriteLevel = mostPlayed.0
		} else {
			favoriteLevel = "N/A"
		}
		...
		let statsData: [(title: String, value: String)] = [
			...
			("Games (Castle):", "\(gamesPlayedLevel1)"),
			("Games (Chasm):", "\(gamesPlayedLevel2)"),
			("Games (Silo):", "\(gamesPlayedLevel3)")
		]
		...
		for (index, data) in statsData.enumerated() {
			...
			if index < statsData.count - 1 {
				let separator = SKSpriteNode(imageNamed: "separator")
				separator.position = CGPoint(x: frame.midX, y: yPosition - (rowHeight / 1.55))
	...
	override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
		...
		for touch in touches {
			...
			if touchedNode.name == "close_button" {
				Audio.shared.playSFX(sound: "button_click")
				Animations.shared.shrink(node: closeButton)
				isButtonTouched = "close_button"
			}
		}
	}
	...
	override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
		...
		for touch in touches {
			...
			if touchedNode.name == "Music Button" && isButtonTouched == "Music Button" {
				...
				SavedSettings.shared.isMusicMuted.toggle() // default launch is set to false. Make sure the values are not interferring with what the proper values of setMusicSettings needs to be set to. Like if SavedSettings is set to true and when you toggle after a new launch, will it be set to true again when is needs to be set to false? --- It looks like that's not the case and is working as intended, double check though
				print("isMM again \(isMusicMuted)")
				Animations.shared.expand(node: musicButton)
				
				if SavedSettings.shared.isMusicMuted {
			...
			if touchedNode.name == "sound_button" && isButtonTouched == "sound_button" {
				SavedSettings.shared.isSoundMuted.toggle() // toggles bool for sounds
			...
			if touchedNode.name == "Controls Button" && isButtonTouched == "Controls Button" {
				SavedSettings.shared.areControlsHidden.toggle()
				Animations.shared.expand(node: controlsButton)
				
				if SavedSettings.shared.areControlsHidden {
			...
			if touchedNode.name == "close_button" && isButtonTouched == "close_button" {
				closeMenu()
			...
			} else if touchedNode.name != "close_button" && isButtonTouched == "close_button" {
				Animations.shared.expand(node: closeButton)
			}
