...
class GameScene: SKScene, SKPhysicsContactDelegate {
	...
	var homeButton = SKSpriteNode(imageNamed: "game_over_home_button")
	...
	var backgroundArrayL1 = [SKSpriteNode]()
	var backgroundArrayL2 = [SKSpriteNode]()
	var backgroundArrayL3 = [SKSpriteNode]()
	
	var wallLeftArray = [SKSpriteNode]()
	var wallRightArray = [SKSpriteNode]()
	
	var lastBackgroundPositionL1: CGFloat!
	var lastBackgroundPositionL2: CGFloat!
	...
	var firstBackgroundL3: SKTexture!
	
	var secondBackgroundL1: SKTexture!
	var secondBackgroundL2: SKTexture!
	var secondBackgroundL3: SKTexture!
	
	var thirdBackgroundL1: SKTexture!
	...
	var currentPlaneSpeedY: CGFloat = 0 // Will store 'modY'
	...
	override func didMove(to view: SKView) {
		...
		lastBackgroundPositionL1 = view.center.y * 3.5 // 1477.0 -- need to figure maybe a better offset to initalize background spawn position
		lastBackgroundPositionL2 = view.center.y * 3.5
		lastBackgroundPositionL3 = view.center.y * 3.5
	...
	func preGame() {
		...
		for node in backgroundArrayL1 {
			node.color = .darkGray
			node.colorBlendFactor = 0.65
		}
		
		for node in backgroundArrayL2 {
			node.color = .darkGray
			node.colorBlendFactor = 0.65
		}
		
		for node in backgroundArrayL3 {
			node.color = .darkGray
			node.colorBlendFactor = 0.65
		}
		...
		if SavedSettings.shared.firstTimePlaying != false {
	...
	func initiateTextures() {
		...
		switch theme {
		case "level_1":
			firstBackgroundL1 = SKTexture(imageNamed: "castle_background_1")
			secondBackgroundL1 = SKTexture(imageNamed: "castle_background_2")
			thirdBackgroundL1 = SKTexture(imageNamed: "castle_background_3")
		...
		case "level_3":
			firstBackgroundL1 = SKTexture(imageNamed: "silo_background_1")
			secondBackgroundL1 = SKTexture(imageNamed: "silo_background_2")
			thirdBackgroundL1 = SKTexture(imageNamed: "silo_background_3")
	...
	func createPlane() {
		...
		plane = SKSpriteNode()
		plane.texture = planeTexture
		plane.position = CGPoint(x: frame.midX / 2, y: (backgroundArrayL1.first?.position.y)! + ((backgroundArrayL1.first?.size.height)! * 0.145))
		plane.anchorPoint = CGPoint(x: 0.5, y: 0.5)
		plane.zPosition = 5
		plane.alpha = 1
	...
	func createLabels() {
		...
		scoreLabel = SKLabelNode(fontNamed: "Paper Plane Font")
	...
	func createBackground() {
		...
		let backgroundL1 = SKSpriteNode()
		backgroundL1.anchorPoint = CGPoint(x: 0.5, y: 0.5)
		backgroundL1.zPosition = -5
		backgroundL1.size = CGSize(width: frame.size.width, height: frame.size.width * 2.5)
		backgroundArrayL1.append(backgroundL1)
		
		let backgroundL2 = SKSpriteNode()
		backgroundL2.anchorPoint = CGPoint(x: 0.5, y: 0.5)
		backgroundL2.zPosition = -6
		backgroundL2.size = CGSize(width: frame.size.width, height: frame.size.width * 2.5)
		backgroundArrayL2.append(backgroundL2)
		
		let backgroundL3 = SKSpriteNode()
		backgroundL3.anchorPoint = CGPoint(x: 0.5, y: 0.5)
		backgroundL3.zPosition = -7
		backgroundL3.size = CGSize(width: frame.size.width, height: frame.size.width * 2.5)
		backgroundArrayL3.append(backgroundL3)
		
		// Doesn't get called at the same time as setBackground in createPlatforms when target score is reached
		
		for background in backgroundArrayL1 {
			background.texture = backgroundTextureL1
		}
		
		for background in backgroundArrayL2 {
			background.texture = backgroundTextureL2
		}
		
		for background in backgroundArrayL3 {
			background.texture = backgroundTextureL3
		}
		
		var yPositionBGL1 = lastBackgroundPositionL1 - backgroundL1.size.height
		var yPositionBGL2 = lastBackgroundPositionL2 - backgroundL2.size.height
		var yPositionBGL3 = lastBackgroundPositionL3 - backgroundL3.size.height
		
		backgroundL1.position = CGPoint(x: view!.center.x, y: yPositionBGL1)
		backgroundL2.position = CGPoint(x: view!.center.x, y: yPositionBGL2)
		backgroundL3.position = CGPoint(x: view!.center.x, y: yPositionBGL3)
		
		//        print("view.center.y \(view!.center.y)")
		
		addChild(backgroundL1)
		nodeArray.append(backgroundL1)
		
		// may need more if-else code further down for potential optional errors
		
		if backgroundL2.texture != nil {
			addChild(backgroundL2)
			nodeArray.append(backgroundL2)
		}
		
		if backgroundL3.texture != nil {
			addChild(backgroundL3)
			nodeArray.append(backgroundL3)
		}
	}
	...
	func gameOver() {
		...
		if noClip == false {
			SavedData.shared.saveGameResults(score: score)
		}
		
		// maybe look into looping these instead each individual call
		...
		bestScore.text = "\(SavedData.shared.getHighscore())"
		bestScore.alpha = 0
		...
		bestScoreShadow.text = "\(SavedData.shared.getHighscore())"
		...
		if score >= 30 {
			if SavedSettings.shared.areAdsRemoved == true || !(gamesUntilAd <= 0) { // change back to false for live build
	...
	override func update(_ currentTime: TimeInterval) {
		...
		// SAFETY CAP: Prevent huge jumps if the app was backgrounded or lagged
		// If dt is larger than 0.1 (10fps), treat it as 0.1
		if dt > 0.1 {
			dt = 0.1
		}
		
		
		lastBackgroundPositionL1 = backgroundArrayL1.last?.position.y
		lastBackgroundPositionL2 = backgroundArrayL2.last?.position.y
		lastBackgroundPositionL3 = backgroundArrayL3.last?.position.y
...
//		        playerCamera.setScale(2)
		...
		let parallaxFactorL1: CGFloat = 0.05 // Moves very slowly (Feels closest)
		let parallaxFactorL2: CGFloat = 0.15 // Moves medium
		let parallaxFactorL3: CGFloat = 0.30 // Moves fastest (Feels farthest)

		
		let worldSpeed = self.scene?.speed ?? 1.0
		
		// Calculate distance to move this frame
		let moveAmountL1 = (currentPlaneSpeedY * parallaxFactorL1) * CGFloat(dt) * CGFloat(worldSpeed)
		let moveAmountL2 = (currentPlaneSpeedY * parallaxFactorL2) * CGFloat(dt) * CGFloat(worldSpeed)
		let moveAmountL3 = (currentPlaneSpeedY * parallaxFactorL3) * CGFloat(dt) * CGFloat(worldSpeed)
		
		// Apply to all active nodes
		if gameHasStarted && gameState != 1 && !gameIsPaused {
			for node in backgroundArrayL1 { node.position.y -= moveAmountL1 }
			for node in backgroundArrayL2 { node.position.y -= moveAmountL2 }
			for node in backgroundArrayL3 { node.position.y -= moveAmountL3 }
		}
		...
		if (backgroundArrayL1.last?.position.y)! + 480 > plane.position.y {
		...
		// Extra check for safety if the physics engine fails to detect contact
		
		if isPlaneDestroyed == false {
			if plane.position.x < frame.minX - 50 || plane.position.x > frame.maxX + 50 {
				isPlaneDestroyed = true
				destroyPlane()
			}
		}
	...
	override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
		...
		for touch in touches {
			...
			if touchedNode.name == "close" {
				...
				SavedSettings.shared.firstTimePlaying = false
