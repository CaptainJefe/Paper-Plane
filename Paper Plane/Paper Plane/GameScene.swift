...
//  }
  ...
  func createBackground() {
    ...
    let moveBackgroundL1 = SKAction.moveBy(x: 0, y: -10, duration: 1)
    let moveBackgroundL2 = SKAction.moveBy(x: 0, y: -25, duration: 1)
    let moveBackgroundL3 = SKAction.moveBy(x: 0, y: -40, duration: 1)
    let repeatMoveL1 = SKAction.repeatForever(moveBackgroundL1)
    let repeatMoveL2 = SKAction.repeatForever(moveBackgroundL2)
    let repeatMoveL3 = SKAction.repeatForever(moveBackgroundL3)
    
    backgroundL1.run(repeatMoveL1)
    backgroundL2.run(repeatMoveL2)
    backgroundL3.run(repeatMoveL3)
  }
  ...
  override func update(_ currentTime: TimeInterval) {
    ...
    lastBackgroundPositionL1 = backgroundArrayL1.last?.position.y // Doesn't work when placed in createBackground() for some reason despite the logic feeling right
